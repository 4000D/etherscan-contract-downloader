...
program
  ...
  .argument("<contractAddress>", "Verified contract address to download source code")
  .option("-d, --dir <dir>", "Directory to save source code")
  .action(async (networkName, contractAddress, options) => {
    ...
    if (!isAddress(contractAddress)) {
      throw new Error(`Contract address ${contractAddress} is not valid`);
    }

    console.log('Downloading source code for contract "%s" on network "%s"', contractAddress, networkName);

    const res: any = await axios.get(network.endpoint(contractAddress), {}).then((res) => res.data);

    const sourceCodes = res.result;

    if (typeof sourceCodes === "string" && sourceCodes === "Invalid API Key") {
